...
import { setBotName, setDescription, setPrompt, setLoading } from '../../store/actions/botActions';
...
const CreateBot = () => {
    ...
    const { botName, description, prompt, loading } = useSelector((state) => state.bot);
        ...
        <form className="flex flex-col gap-[3vw] pb-[1vw] relative ">
            {[
                ...
                },
                {
                    ...
                    placeholder: '',
                    Component: 'textarea',
                    descriptionText: "Enter your chatbot's description, including its main purpose and tasks.(eg. A customer support assistant that answers FAQs and resolves issues, or a travel booking helper that finds and books flights and hotels)",
                },
                {
                    label: 'Prompt*',
                    type: 'text',
                ...
                (
                    ...
                    <div key={index} className="flex w-full justify-between relative gap-[4vw] px-[5vw]">
                        ...
                        {Component === 'input' ? (
                            <input
                                ...
                                value={value}
                                onChange={onChange}
                                className={`border-[0.052vw] border-zinc-300 w-full px-[1.25vw] py-[1.2vh] rounded-[0.417vw] ${className} ${theme === 'dark'
                            ...
                            <textarea
                                ...
                                value={value}
                                onChange={onChange}
                                className={`Hmd border-[0.052vw] w-full border-zinc-300 px-[1.25vw] pt-[1.6vh] pb-[1.6vh] rounded-[0.417vw]  overflow-hidden ${theme === 'dark'
                                    ? 'bg-[#1F222A] text-white'
                                    : 'bg-white text-black'
                                    }`}
                                placeholder={placeholder}
    ...
    return (
        <div className={`w-full h-screen relative flex flex-col justify-center items-center ${theme === 'dark' ? 'bg-[#1D2027] text-white' : 'bg-[#F2F4F7] text-black'}`}>
            ...
            <div className={`w-[75vw] overflow-y-auto mx-auto h-[80vh] mt-[2vh] rounded-[.9vh] shadow-xl py-[3.5vh] px-[2.7vh] relative  ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}>
                <h1 className="text-2xl font-semibold text-left pb-[3vh]">{loading ? <Skeleton width={200} /> : 'Create a Bot'}</h1>
            ...
            <div className={`w-full absolute bottom-0 h-[6.5vh] ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}>
                <div className='w-full h-full flex justify-end items-center gap-[2vw] px-[3vw] pr-[13vw] '>
                    <GradientButton
                        text='back'
                        onClick={prevHandler}
                    />
                    <GradientButton
                        text='continue'
                        onClick={nextHandler}
                        className='bg-[#702963] text-white py-[.3vw] '
                    />
                </div>
            </div>
        </div>

...
const DemoBot = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        await fetch(
            `${urlFetch}/tester/chat?user_input=${inputValue}&conversation_id=${sessionUUID}`,
            {
                method: 'GET',
                headers: {
        ...
        .then((res)=>res.text())
        .then((res)=>{setChatLog((pre)=>[...pre,{type:'bot',message:res}]);console.log(res)})
        setTimeout(() => {
            setInputValue('')
        }, 1000)
    }
    ...
    const handleOptionSubmit = (value) => {
        ...
        setChatLog((prevChatLog) => [...prevChatLog, { type: 'user', message: value }]);
        setInputValue('');
    };
  ...
  return (
    <>
         <div className={`bg-white rounded-[.8vw] flex flex-col justify-between min-w-[30vw] min-h-[65vh] max-w-[30vw] max-h-[65vh] absolute overflow-hidden`}>
        <Chatbot/>
         </div> 
        ...
        <div className={`flex flex-col gap-[5px] mb-[10px] max-w-[70%]`}>
            {
                options?.map((option, index) => (
                    <div 
                        key={index} 
                        className={`rounded-xl p-[1.5px] cursor-pointer bg-gradient-to-r from-indigo-500 to-blue-500 hover:opacity-[0.8] ${!value ? 'opacity-[1]' : value != option && 'opacity-[0.5]'}`}
                    >
                        <button 
                            type='button'  
        ...
        <div className={`flex flex-col gap-[5px] mb-[10px] max-w-[70%]`}>
            {
                options?.map((option, index) => (
                    <div 
                        key={index} 
                        className={`rounded-xl w-[100%] p-[1.5px] cursor-pointer bg-gradient-to-r from-indigo-500 to-blue-500 hover:opacity-[0.8] ${inputValue != option && 'opacity-[0.5]'} `}
                    >    
                        <div className={`bg-white bg-opacity-[0.9] w-full h-full rounded-xl px-3 py-1`}>
                            <div className={`w-full bg-gradient-to-r from-indigo-500 to-blue-500 text-transparent inline-block bg-clip-text font-semibold `}>
                                <input
                                    type="radio"
                                    name='radio'
                                    id={option}
                                ...
                                />
                                <label htmlFor={option} className='break-words'>{option}</label>
                            </div>
                        </div>
                    </div>
                ))
...
export const ReplyInput = ({ setValue, btnAction }) => {
    return (
        <div className='flex flex-col gap-[5px] mb-[10px] max-w-[70%]'>
            <div className={`rounded-xl w-[100%] p-[1.5px] cursor-pointer bg-gradient-to-r from-indigo-500 to-blue-500 hover:opacity-[0.8]`}>

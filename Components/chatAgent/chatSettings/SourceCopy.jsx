...
const Source = () => {
    ...
    return (
        <div className={`flex flex-col justify-start items-start px-[2vw] w-full h-[88.5vh]`}>
            <div className={`flex items-start justify-center py-[4vh] gap-[2vw] pl-[8vw] pr-[3vw] w-full h-full`}>
                <div className={`flex flex-col justify-start min-h-full w-[80%] py-[4vh] px-[1vw] rounded-[.5vw] ${theme === "dark" ? 'bg-[#1A1C22] text-white' : 'bg-white text-black'}`}>
                    <h1 className='px-[2.5vw] text-[1.3vw] font-semibold pb-[1vh]'>Data Source</h1>
                            ...
                            {selectedSection === 0 ? (
                                ...
                                <div className='w-[80%] flex flex-col items-center justify-start'>
                                    <input
                                        ...
                                        type='url'
                                        id="url"
                                        className={`text-[1vw] border-[0.052vw] w-full border-zinc-300 px-[1.3vw] py-[.5vh] rounded-[.4vw] ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}
                                        placeholder="Enter URLs"
                                    />
                                    <div className='flex flex-col gap-[.5vw] text-[1vw] mt-[2vh] w-[100%]'>
                                        {pastedUrl.length > 0 && pastedUrl.map((url, index) => (
                                            <div key={index} className='h-[4vh] w-full border-[1px] border-zinc-300 px-[1vw] py-[.5vh] text-[.7vw] rounded-[.4vw] flex justify-between items-center'>
                                ...
                                <textarea
                                    ...
                                    value={RawText}
                                    id="rawText"
                                    className={`text-[1vw] border-[0.052vw] w-[35vw] border-zinc-300 px-[1.3vw] pb-[1.8vh] pt-[1.8vh] rounded-[.5vw] overflow-hidden ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}
                ...
                <div className={`flex flex-col gap-[1vw] justify-center items-center w-[20%] py-[4vh] px-[1vw] rounded-[.5vw] ${theme === "dark" ? 'bg-[#1A1C22] text-white' : 'bg-white text-black'}`}>
                    ...
                    <h6 className={`font-semibold text-[.9vw] pt-[.7vw]`}>Total characters detected</h6>
                    <p className={`text-[.8vw] pb-[.7vw]`}><span className={`font-semibold`}>{totalWordCount}</span> /1,000,000 limit</p>
                    <button className={`text-white font-semibold text-[1vw] bg-pink-600 px-[1vw] py-[.6vh] rounded-md`}>Update</button>
                </div>
            </div>
        </div>
    );

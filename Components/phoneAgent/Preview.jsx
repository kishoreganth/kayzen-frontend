...
const Preview = () => {
  ...
  const { gender, voice, stability, similarity, exaggeration, speakerBoost, advancedSetting } = useSelector((state) => state.voice)
  const { phoneAgentName, phoneAgentPurpose, companyName, companyBusiness, companyServices } = useSelector((state) => state.phoneAgent)
  ...
  const callSummaries = [
    { contact: '+91 405 555-0128', duration: '20s', date: '05 Jan 2024 02:34 PM', to: '' },
    { contact: '+91 405 555-0128', duration: '20s', date: '05 Jan 2024 02:34 PM', to: '' },
    { contact: '+91 405 555-0128', duration: '20s', date: '05 Jan 2024 02:34 PM', to: '' },
    { contact: '+91 405 555-0128', duration: '20s', date: '05 Jan 2024 02:34 PM', to: '' },
    { contact: '+91 405 555-0128', duration: '20s', date: '05 Jan 2024 02:34 PM', to: '' },
  ...
  return (
    <div className={`w-full h-screen relative flex flex-col justify-center items-center ${theme === 'dark' ? 'bg-[#1D2027] text-white' : 'bg-[#F2F4F7] text-black'}`}>
      ...
      <div className={`w-[75vw] overflow-y-auto mx-auto h-[80vh] mt-[5vh] mb-[5vh] rounded-[.9vh] shadow-xl py-[3.5vh] px-[2.7vh]  ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}>
        <div className='w-full h-[0.4vh]/2 flex items-start justify-around mb-[1vw] px-[3vw]'>
          <div className='flex flex-col gap-[2vw]'>
            <h1 className='text-[1.3vw] capitalize font-semibold'>preview agent</h1>
            <h5 className='text-[.8vw] w-10/12 font-medium text-zinc-500'>
              To preview the phone agent, please enter any phone number to receive a call from the AI phone agent. You can test the phone agent before deployment. You can find the summary of the call summary below or after you finish saving.
            </h5>
            <h6 className='text-[.725vw] font-medium text-zinc-500'>In case of further changes, go back to edit the agent.</h6>
          </div>
          ...
          <div className='w-full flex flex-col items-start gap-[2vw] bg-white rounded-[0.417vw] m-[1vw] p-[1.5vw] shadow-[0_5px_30px_-10px_rgba(0,0,0,0.3)]'>
            <div className='flex justify-between w-full'>
              <label className='text-[1vw] py-[1.5vh] px-[1vw] rounded-[0.417vw] '>Phone</label>
              <div className='select-container flex w-[18vw] gap-[.5vw] items-center justify-center'>
                <select
                  ...
                  className={`${theme === 'dark' ? 'bg-[#1A1C22] text-white' : 'bg-zinc-200 text-black'} Hmd px-[1vw] py-[1.5vh] rounded-[0.417vw] select cursor-pointer`}
                  name="countryCode"
                  id="countryCode"
                >
            ...
            <div className="flex justify-between w-full">
              <label className='text-[1vw] py-[1.5vh] px-[1vw] rounded-[0.417vw] '>Name</label>
              <input className=' text-[1vw] border-[0.052vw] bg-transparent border-zinc-400 w-[18vw] py-[1.5vh] px-[1vw] rounded-[0.417vw] '
                type="text"
                placeholder="Name"
            ...
            <div className="flex justify-between w-full">
              <label className='text-[1vw] py-[1.5vh] px-[1vw] rounded-[0.417vw] '>Purpose</label>
              <input
            ...
            <div className='flex justify-end mx-auto mb-[1vh]'>
              <GradientButton text='call now' className=' contentButton bg-gradient-to-r from-[#EB1CD6]  to-[#F4A36F] text-white px-[3vw] text-[1vw] ' onClick={() => makeCall()} />
            </div>
          </div>
        </div>

        <div className="w-full h-[0.4vh]/2  flex justify-center items-center">
          <div className={`w-[80vw] h-auto  rounded-[0.417vw] px-[2vw] py-[2vw] relative ${theme === 'dark' ? 'bg-[#1D2027] text-white' : 'bg-[#F2F4F7] text-black'}`}>
            <h1 className='text-[1.042vw] capitalize font-medium mb-[1.6vh]'> Call Summary</h1>
      ...
      <div className={`w-full absolute right-0 bottom-0 h-[6.5vh] ${theme === "dark" ? 'bg-[#1F222A] text-white' : 'bg-white text-black'}`}>
        <div className='w-full h-full flex justify-end items-center gap-[2vw] px-[3vw] '>
          <GradientButton
            text='back'
            onClick={() => navigate.push('/workspace/agents/phone/voicesetting')}
          />
          <GradientButton
            text='finish'
            onClick={() => navigate.push('/workspace/agents')}
